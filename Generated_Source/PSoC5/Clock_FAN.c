...
void Clock_FAN_Start(void) 
{
    ...
    Clock_FAN_CLKEN |= Clock_FAN_CLKEN_MASK;
	Clock_FAN_CLKSTBY |= Clock_FAN_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_FAN_Stop
********************************************************************************
*
* Summary:
...
void Clock_FAN_Stop(void) 
{
    ...
    Clock_FAN_CLKEN &= (uint8)(~Clock_FAN_CLKEN_MASK);
	Clock_FAN_CLKSTBY &= (uint8)(~Clock_FAN_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_FAN_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_FAN_CLKEN & Clock_FAN_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_FAN_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_FAN_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_FAN__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_FAN_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_FAN_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_FAN_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_FAN_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_FAN_GetSourceRegister();
    uint16 oldDivider = Clock_FAN_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_FAN_CLKEN & Clock_FAN_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_FAN_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_FAN_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_FAN_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_FAN_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_FAN_MOD_SRC |= modeBitMask & (uint8)Clock_FAN_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_FAN_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_FAN_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_FAN_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_FAN_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_FAN_MOD_SRC & (uint8)(Clock_FAN_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_FAN_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_FAN_GetDividerRegister();
    uint8 oldSrc = Clock_FAN_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_FAN_MOD_SRC |= CYCLK_SSS;
        Clock_FAN_MOD_SRC =
            (Clock_FAN_MOD_SRC & (uint8)(~Clock_FAN_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_FAN_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_FAN_MOD_SRC & Clock_FAN_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_FAN_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_FAN_PHASE = clkPhase & Clock_FAN_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_FAN_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_FAN_PHASE & Clock_FAN_PHASE_MASK;
}
